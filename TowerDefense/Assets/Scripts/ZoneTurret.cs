            ...
            if (entity.CompareTag("Entity"))
            {
                if (!entity.GetComponent<Enemy>().mage)
                {
                    if (!entity.GetComponent<Enemy>().onFire && FireTurret)
                    {
                        ...
                        GameObject particle = (GameObject) Instantiate(particleAnimation, entity.transform.position,
                            Quaternion.identity);
                        particle.transform.SetParent(entity.transform);
                        particle.transform.LookAt(GameObject.FindWithTag("GameController")
                            .GetComponent<GameManager>().MainCam.transform);
                    }
                    ...
                    if (PoisonTurret)
                    {
                        ...
                        {
                            GameObject particle = (GameObject) Instantiate(particleAnimation,
